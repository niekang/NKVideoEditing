...
#import "NKVideoDecoder.h"
#import <VideoToolbox/VideoToolbox.h>

@interface NKVideoDecoder (){
    uint8_t *_sps;
    NSUInteger _spsSize;
    uint8_t *_pps;
    NSUInteger _ppsSize;
    CMVideoFormatDescriptionRef _decodeDescRef;
}
...
@implementation NKVideoDecoder

- (instancetype)initWithConfig:(NKVideoEncoderConfig *)config {
    self = [super init];
    if (self) {
        self.decodeQueue = dispatch_queue_create("com.nk.decode.video.queue", DISPATCH_QUEUE_SERIAL);
        self.callbackQueue = dispatch_queue_create("com.nk.decode.video.callback.queue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}

- (BOOL)createDecoder {
    if (_decodeSesion) return self;
    const uint8_t *paramtersSetPointers[2] = {_sps, _pps};
    int NALUHeaderLenght = 4;
    const size_t parqamtersSetSizes[2] = {_spsSize, _ppsSize};
    /**
     根据sps pps设置解码参数
     ...
     return 状态
     */
    OSStatus status = CMVideoFormatDescriptionCreateFromH264ParameterSets(kCFAllocatorDefault, 2, paramtersSetPointers, parqamtersSetSizes, NALUHeaderLenght, &_decodeDescRef);
    if (status != noErr) {
        NSLog(@"CMVideoFormatDescription create H264ParameterSets(sps, pps) failed status= %d", (int)status);
        return NO;
    }
    /*
     解码参数:
    ...
    * kCVPixelBufferWidthKey/kCVPixelBufferHeightKey: 视频源的分辨率 width*height
     ...
     */
    status = VTDecompressionSessionCreate(kCFAllocatorDefault, _decodeDescRef, NULL, (__bridge CFDictionaryRef _Nullable)(destinationPixBufferAttrs), &callbackRecord, &_decodeSesion);
    //判断一下status
    if (status != noErr) {
        NSLog(@"VTDecompressionSessionCreate failed status= %d", (int)status);
        return NO;
    }
    //设置解码会话属性(实时编码)
...
- (CVPixelBufferRef)decode:(uint8_t *)frame withSize:(uint32_t)frameSize {
    ...
    /*
     ...
     参数9: sampleBuffer对象
     */
    status = CMSampleBufferCreateReady(kCFAllocatorDefault, blockBuffer, _decodeDescRef, 1, 0, NULL, 1, sampleSizeArray, &sampleBuffer);
...
void videoDecompressionOutputCallback(void * CM_NULLABLE decompressionOutputRefCon,
    ...
    *outputPixelBuffer = CVPixelBufferRetain(imageBuffer);
    ...
    //获取self
    NKVideoDecoder *decoder = (__bridge NKVideoDecoder *)(decompressionOutputRefCon);
